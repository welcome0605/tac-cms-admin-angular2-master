...
const FormBuilder =  (opts, element) =>{
    ...
    const kinds = (<any>config).kinds = h.processKinds(opts.kinds);
    ...
    /**
     * Add data for field with slide images
     *
     * @param  {Object} fieldData
     * @return {String} field images markup
     */
    let fieldSlideImages = function (fieldData) {
        
      let {type, values, name} = fieldData;
      let slideshowActions = [
        m('a', i18n.addSlideImage, {className: 'add add-slide-image'})
      ];
      let fieldImages = [
        m('label', i18n.selectSlideImages, {className: 'false-label'})
      ];
      ...
      if ( !values || !values.length ) {
        ...
        values = defaultImageCount.map(function (index) {
          return optionDataTemplate();
        });

      } else {
        // ensure option data is has all required keys
        values.forEach(option => Object.assign({}, option));
      }

      const slideshowActionsWrap = m('div', slideshowActions, {className: 'option-actions'});
      const images = m('ol', values.map((option, index) => createFieldSlideImages(`${name}-${index}`, option)), {className: 'sortable-options'});
      const imagesWrap = m('div', [ images, slideshowActionsWrap ], {className: 'sortable-options-wrap'});

      fieldImages.push(imagesWrap);

      return m('div', fieldImages, {className: 'form-group field-options'}).outerHTML;
    };

    /**
     * Generate padding inputs for image/video, text, etc
     * @param  {Object} values
     * @return {String}
     */
    const paddingAttribute = (values) => {
        let attributefield: any = '';
        const attributes = ['top', 'right', 'bottom', 'left'];
        for ( let i = 0; i < 4; i++)
        {
            let inputConfig: any = {
                name: `padding-${attributes[i]}`,
                className: `fld-padding-${attributes[i]} form-control`,
                id: `padding-${attributes[i]}-${data.lastID}`
            };
            let attributeLabel = m('label', attributes[i]).outerHTML;
            inputConfig.value = (values[`padding${utils.capitalize(attributes[i])}`] || 0).toString();
            inputConfig.type = 'number';
            let inputWrap = `
                <div class="input-wrap input-group">
                    ${attributeLabel}
                    <input ${utils.attrString(inputConfig)}>
                    <div class="input-group-append">
                        <span class="input-group-text">px</span>
                    </div>
                </div>`;
            attributefield += m('div', [inputWrap], {
                className: `form-group padding-${attributes[i]}-wrap`
            }).outerHTML;
        }
        let paddingLabel = m('label', 'Padding').outerHTML;
        return m('div', [paddingLabel, attributefield], {
            className: `form-group padding-wrap`
        }).outerHTML;
    };
...
//                'maxlength',
    ...
    let advFields = values => {
        ...
        const advFieldMap = {
            ...
            slide_images: () => fieldSlideImages(values),
            padding: () => paddingAttribute(values),
            kind: () => selectAttribute('kind', values, kinds[type])
        };
        let key;
    ...
    // Select field html, since there may be multiple
    let createFieldSlideImages = function (name, imageData) {
        let imageDataOrder = [
            'source'
        ];
        let imageInputs = [];
        let optionTemplate = {source: ''};
    
        imageData = (<any>Object).assign(optionTemplate, imageData);
    
        for (let i = imageDataOrder.length - 1; i >= 0; i--) {
            let prop = imageDataOrder[i];
            if (imageData.hasOwnProperty(prop)) {
                let attrs: any = {
                    type: 'text',
                    className: 'image-' + prop,
                    value: imageData[prop],
                    name: name + '-image'
                };
    
                attrs.placeholder = i18n[`placeholder.${prop}`] || '';
                imageInputs.push(m('input', null, attrs));
            }
        }
        ...
        imageInputs.push(utils.markup('a', i18n.remove, removeAttrs));
    
        let field = utils.markup('li', imageInputs);
    ...
    // delete slide images
    $stage.on('click touchstart', '.remove-slide-image', e => {
        let _event : any = e;
        let $field = jQuery(_event.target).parents('.form-field:eq(0)');
        let field = $field[0];
        let type: any = field.getAttribute('type');
        ...
        if (optionsCount <= 2 && !type.includes('checkbox')) {
            ...
            $option.slideUp('250', () => {
                ...
                $(`.image-dropify#${(<any>$option.find('input')[0]).name}-wrapper`).remove();
                // h.updatePreview($field);
                // h.save.call(h);
            });
        }
    ...
    $stage.on('imageChanged', '.image-dropify', e => {
        let _event: any = e;

        var event = new CustomEvent('corruptImageListChanged',{bubbles: true, detail: _event.target.id});
        element.dispatchEvent(event);
        // $(element).trigger('corruptImageListChanged', _event.target.id);
    ...
    // Attach a callback to add new options
    $stage.on('click', '.add-slide-image', function (e) {
        e.preventDefault();
        let $optionWrap = $(e.target).closest('.field-options');
        let $lastOption = $('.image-source:eq(-1)', $optionWrap);
        let name = $lastOption.attr('name');

        $('.sortable-options', $optionWrap)
            .append(createFieldSlideImages(name, false));
    });
    ...
    formBuilder.actions = {
        ...
        setBackground: background => {
            ...
            Helpers.setBackground(background);
        },
        setLang: async locale => {
            await I18N.setCurrent.call(I18N, locale);
            d.empty(element);
            let formBuilder = FormBuilder(originalOpts, element);
