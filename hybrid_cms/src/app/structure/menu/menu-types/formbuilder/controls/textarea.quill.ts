...
export default class controlQuill extends controlTextarea {
  ...
  onRender(evt) {
    ...
    const Delta = (<any>window).Quill.import('delta');
      (<any>window).fbEditors.quill[this.id] = {};
    let editor = (<any>window).fbEditors.quill[this.id];
    editor.instance = new (<any>window).Quill(this.field, {
      modules: {
        toolbar: [
          [{'header': [1, 2, false]}],
    ...
    if (value) {
      editor.instance.setContents((<any>window).JSON.parse(this.parsedHtml(value)));
    }
