...
export default class Helpers {
    ...
    save(stage = null) {
        ...
        $('input#hidden-info').val(data.formData);
    ...
    updatePreview($field) {
        ...
        } else if (fieldType == 'slideshow') {
            previewData.values = [];
            $('.sortable-options li', field).each(function (i, $image) {
                let image: any = {};
                image.source = $('.image-source', $image).val();
                image.id = $('.image-source', $image)[0].getAttribute('name');
                previewData.values.push(image);
            });
        }
        ...
        if (controlClass.preProcess) {
            controlClass.preProcess(field, previewData);
        }
    ...
    toggleEdit(fieldId, animate = true) {
        ...
        const deleteBtn = $('.delete-confirm', field);
    ...
    /**
     * Register any kind controls specified in the 'kinds' option, retrieve
     * all defined kinds & build the export kind format
     * @param  {Array} kindOpts
     * @return {Array} kinds
     */
    processKinds(kindOpts) {
        // retrieve a list of all kinds
        let kindDef = control.getRegisteredKinds();

        // reformat the kinds for each type
        let kinds = {};
        for (let type in kindDef) {
            if (kindDef.hasOwnProperty(type)) {
                let formatted = [];
                for (let kind of kindDef[type]) {
                    formatted.push({
                        label: kind,
                        value: kind
                    })
                }
                kinds[type] = formatted;
            }
        }
    ...
    static getBackground() {
        return Helpers.background;
    }

    static setBackground(background_color) {
        
        Helpers.background = background_color;
    }
