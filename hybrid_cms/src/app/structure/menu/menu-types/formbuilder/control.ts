...
export class control {
    ...
    public static kindRegister: any = {};
    public element: any;
  ...
  /**
    ...
    /**
     * Class method to register supported controls and their associated classes
     * @param {Array} kinds - control kind (or array of control kinds) to register
     * against the specifed class
     * @param {Class} controlClass - class to map against the kinds
     */
    static registerKinds(kinds, controlClass,parentType) {

        const prefix = parentType + '.';
        
        // initialise the register
        if (!control.kindRegister) {
            control.kindRegister = {};
        }
        if (!Array.isArray(kinds)) {
            kinds = [kinds];
        }

        // associate the controlClass with each passed control kind
        for (const kind of kinds) {
            control.kindRegister[prefix + kind] = controlClass;
        }
    }
    ...
    /**
     * Looks up the kindRegister & returns registered kinds
     * @return {Array} registered kinds (or kinds)
     */
    static getRegisteredKinds() {
        
        let kinds = [];
        for (const key in control.kindRegister) {
            if (control.kindRegister.hasOwnProperty(key)) {
